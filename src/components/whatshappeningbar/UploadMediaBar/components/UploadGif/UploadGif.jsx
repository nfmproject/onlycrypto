...
export const UploadGif = ({ disabled }) => {
  ...
  return (
    <div>
      <button type="button" disabled={disabled} className="UploadGif__Btn" onClick={toggleGifGrid}>
        <img alt="Upload GIF" src={UpladGifSVG} />
      </button>
      {showGrid && <GifGrid onCloseGifGrid={toggleGifGrid} onGifClick={onGifClick} />}
    </div>
  );
};
